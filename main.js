...
function echoLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [ECHO-MD] ${new Date().toISOString()}: ${message}`);
}
...
echoLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { echoLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                echoLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            echoLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            echoLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                echoLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { echoLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                echoLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await echoPair(number, mockRes);
                } catch (e) { echoLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                echoLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function echoPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `echo_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            echoLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                echoLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            echoLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                echoLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                await echomd(conn);
                echoLog(`Connected: ${sanitizedNumber}`, 'success');
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) echoLog(`Session logged out.`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363426850850275@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    key: { remoteJid: 'status@broadcast', participant: '2349129557631@s.whatsapp.net', fromMe: false, id: createSerial(16).toUpperCase() },
                    message: { contactMessage: {
                        displayName: '© ECHO-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:ECHO-MD BOY\nORG:ECHO-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© ECHO-MD' } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { echoLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { echoLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        echoLog(`ECHO-MD Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await echoPair(req.query.number, res); });
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'ECHO-MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ECHO-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        echoLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { echoLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        echoLog('Auto-reconnect completed', 'success');
    } catch (e) { echoLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
...
process.on('uncaughtException', (err) => {
    echoLog(`Uncaught exception: ${err.message}`, 'error');
});
